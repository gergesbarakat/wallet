...
<title>admin add assigned_room</title>
    ...
    <div class="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="content container-fluid">
                ...
                <!-- Search Filter -->
                <form action="assigned_room-list" method="post" id="filter_inputs">
                    @csrf
                            ...
                            <div class="row filter-row">
                                <div class="col-sm-6 col-md-3">
                                    <div class="form-group">
                                        <label>assigned_room</label>
                                        <select class="form-control" name="assigned_room"
                                            id="assigned_room_assigned_room">
                                            <option value="">Select assigned_room</option>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table table-hover table-center mb-0 service_table">
                                        <?php
                                        $assigned_rooms = DB::table('assigned_rooms')->get()->all();
                                        ...
                                        <tbody>
                                            @foreach ($assigned_rooms as $assigned_room)
                                                <tr>
                                                    <td>{{ $assigned_room->id }}</td>
                                                    <td> {{ DB::table('users')->find($assigned_room->user_id)->email }}
                                                    </td>
                                                    <td> {{ DB::table('rooms')->find($assigned_room->room_id)->name }}
                                                    </td>
                                                        ...
                                                        <div data-toggle="tooltip"
                                                            ...
                                                            <div class="status-toggle">
                                                                <input id="status_1" class="check change_Status_user"
                                                                    ...
                                                                    {{ $assigned_room->status == 'active' ? 'checked' : '' }}
                                                                    disabled>
                                                                <label for="status_1"
                                                    ...
                                                    <td>{{ $assigned_room->created_at }}</td>
                                                    <td>{{ $assigned_room->updated_at }}</td>

                                                    <td>
                                                        <a href=" {{ route('assigned-rooms.edit', $assigned_room->id) }}"
                                                            class="btn btn-sm bg-success-light mr-2">
                                                            <i class="far fa-edit mr-1"></i> Edit
                                                        </a>
                                                        <a onclick="$('#delete_assigned_room_{{ $assigned_room->id }}').submit()"
                                                        ...
                                                        <form id="delete_assigned_room_{{ $assigned_room->id }}"
                                                            method="post"
                                                            action="{{ route('assigned-rooms.destroy', $assigned_room->id) }}"
                                                            style="display: none">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
