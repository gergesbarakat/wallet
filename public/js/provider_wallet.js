...
if(stripe_key != '') {
    ...
    sub_btn.addEventListener('click', function(event) {
        ...
        stripe.createToken(card,{'currency': currency_val}).then(function(result) {
            if (result.error) {
                ...
                var data="Token="+tokens+"&amount="+stripe_amt+"&currency_val="+currency_val+"&tokenid="+tokenid+"&csrf_token_name="+csrf_token+"&paytype=paypal";
                $.ajax({
                    url: base_url+'api/api/wallet_withdraw',
                    data:data,
                    type: 'POST',
                    dataType: 'JSON',
                    success: function(response){    
                        if(response.response.response_code==200 || response.response.response_code=='200'){
                            Swal.fire({
                                ...
                                text: "Wallet amount has been credited!",
                                icon: "success",
                                button: "okay",
                                closeOnEsc: false,
                            ...
                            Swal.fire({
                                title: "Wallet Amount Not Succeed..Wallet Amount Not Succeed...",
                ...
                success: function(response){
                    if(response.status) {
                        ...
                        if(payment_type == "stripe") { 
                            ...
                            if(stripe_key == '' || stripe_key == undefined) {
                                Swal.fire({
                                    title: "Empty Key",
                                    text: "Please Enter Payment api key",
                                    icon: "error",
                        ...
                        if(payment_type=="iyzico") {
                            ...
                            } else if(stripe_amt =='' || stripe_amt < 10000){
                                iyzico_details(response.results);
                            }
                        }
                        //midtrans
                        if(payment_type=="midtrans") {
                            ...
                            if(stripe_amt =='' || stripe_amt > 10000){
                            
                                alert("Wallet Amount should be greater than or equal to 10,000");
                                return false;
                            }else{

                                midtrans_details(response.results);
                            }
                         }
                        ...
                        Swal.fire({
                            ...
                            window.location.href = base_url+'payout-settings';
                        });
                    }
                }
            });
    ...
    //iyzico_withdraw
    function iyzico_details(prev_response) {
        ...
        var req_url = base_url+'user/dashboard/iyzico_withdraw/'+stripe_amt;
    ...
    function midtrans_details(prev_response) {
         var userId = prev_response.user_id;
         var tokens=$('#token').val();
         var stripe_amt=$("#wallet_withdraw_amt").val();
         var data="user_id="+userId+"&Token="+tokens+"&amount="+stripe_amt+"&csrf_token_name="+csrf_token;
         $.ajax({
             url: base_url+'user/dashboard/midtrans_withdraw',
             data: data,
             type: 'POST',
             dataType: 'JSON',
