...
function get_user_chat_lists() {
            ...
            if(data!='') {
                ...
                $(res.chat_list).each(function(index, value) {
                    if(value.name !== null) {
                        ...
                        if(value.badge!=0) {
			                if(showBadge == 1) {
					           badge="<span  class='position-absolute badge_col badge-pill bg-yellow-chat chat-bg-yellow '>"+value.badge+"</span>";
			                } else {
 ...
 function history_append_fun(token){
  ...
  if($('#chat-message').val()!=''){
  $.post(base_url+'admin-chat/insert_chat',{fromToken: $('#fromToken').val(),toToken:$('#toToken').val(),content:$('#chat-message').val(),csrf_token_name:csrf_token},
    function(response){
  ...
  $('#submit').attr('disabled',true);
    ...
    $('#chat_counts').text(chat_count);*/
    $(".chat_counts").load(location.href + " .chat_count");
		get_user_chat_lists(); 
	}, 30000);
